...
sum(!complete.cases(all_data)) #only 14 rows have missing data - removed below
...
interactive_map <- leaflet(avgdata_map) %>% addProviderTiles("CartoDB.Positron") %>%
  ...
  addLegend(position = "bottomleft", pal=grey_pal, values=avgdata_map@data$mean_weight, title="Scaling (Higher is Better") %>%
